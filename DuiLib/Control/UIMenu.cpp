...
namespace DuiLib {
		...
		DWORD dwAlignment/* = eMenuAlignment_Left | eMenuAlignment_Top*/)
	{
		...
		if (pOwner == NULL)
		{
			...
			s_context_menu_observer.SetManger(pMainPaintManager);
			if (pMenuCheckInfo != NULL)
				s_context_menu_observer.SetMenuCheckInfo(pMenuCheckInfo);
		}
	...
	void CMenuWnd::Notify(TNotifyUI& msg)
	{
		if( s_context_menu_observer.GetManager() != NULL) 
		{
			...
			{
				s_context_menu_observer.GetManager()->SendNotify(msg, false);
			}
		}
	}
	...
	void CMenuElementUI::CreateMenuWnd()
	{
		...
		m_pWindow->Init(static_cast<CMenuElementUI*>(this), _T(""), POINT(), NULL);
	}
	...
	void CMenuElementUI::DrawItemIcon(HDC hDC, const RECT& rcItem)
	{
		if ( m_strIcon != _T("") )
		{
			...
			if (!(m_bCheckItem && !GetChecked()))
			{
				...
				TDrawInfo diImage;
				diImage.sDrawString = pStrImage;
				CRenderEngine::DrawImage(hDC, m_pManager, m_rcItem, m_rcPaint, diImage);
			}			
		}
	}
	...
	{
		if (!m_bCheckItem || s_context_menu_observer.GetMenuCheckInfo() == NULL )
		return;
		map<CDuiString,bool>::iterator it = s_context_menu_observer.GetMenuCheckInfo()->find(GetName());
		if (it == s_context_menu_observer.GetMenuCheckInfo()->end())
		s_context_menu_observer.GetMenuCheckInfo()->insert(map<CDuiString,bool>::value_type(GetName(),bCheck));
		else
		it->second = bCheck;
	...
	bool CMenuElementUI::GetChecked() const
	{
		...
		if (!m_bCheckItem || s_context_menu_observer.GetMenuCheckInfo() == NULL || s_context_menu_observer.GetMenuCheckInfo()->size() == 0)
		return false;
	...
	void CMenuElementUI::DrawItemExpland(HDC hDC, const RECT& rcItem)
	{
		if (m_bShowExplandIcon)
		{
			...
			TDrawInfo diImage;
			diImage.sDrawString = strBkImage;
			CRenderEngine::DrawImage(hDC, m_pManager, m_rcItem, m_rcPaint, diImage);
		}
	}
	...
	void CMenuElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if( _tcscmp(pstrName, _T("icon")) == 0){
			...
			if (s_context_menu_observer.GetMenuCheckInfo() != NULL && 
				s_context_menu_observer.GetMenuCheckInfo()->find(GetName()) == s_context_menu_observer.GetMenuCheckInfo()->end())
			{
