...
, m_bIsShowShadow(false)
{
	...
	::ZeroMemory(&m_rcShadowPadding, sizeof(RECT));
}
...
void CShadowUI::Update(HWND hParent)
{
	...
	if (m_bIsImageMode)
	{
		RECT rcPaint = {m_rcShadowPadding.left, m_rcShadowPadding.top, nShadWndWid + m_rcShadowPadding.right, nShadWndHei + m_rcShadowPadding.bottom};
...
bool CShadowUI::SetShadowPadding(RECT rcPadding)
{
	m_rcShadowPadding = rcPadding;
